      ...
      {open && (
        <div className="absolute top-full left-0 mt-1 z-50 bg-white border border-gray-200 rounded-lg shadow-lg py-1 min-w-full">
          <button
            ...
            className={`w-full text-left px-3 py-1.5 text-sm hover:bg-gray-50 transition-colors ${value === '' ? 'text-gray-900 font-medium' : 'text-gray-600'}`}
          >
            {allLabel}
          </button>
          {options.map(opt => (
            <button
              ...
              className={`w-full text-left px-3 py-1.5 text-sm hover:bg-gray-50 transition-colors ${value === opt.value ? 'text-gray-900 font-medium' : 'text-gray-600'}`}
            >
              {opt.label}
            </button>
          ))}
