  ...
  return (
    <div className="min-h-screen bg-white text-gray-900 font-sans">
      ...
      <div className="max-w-3xl mx-auto px-6 py-16">
        ...
        <H3>{sl ? '26. člen – Storitev »kot je« in sprejem tveganja' : 'Article 26 – Service "as is" and acceptance of risk'}</H3>
        <P>{sl
          ? 'Naročnik in vsak Uporabnik z registracijo, sprejemom teh Pogojev oziroma uporabo Toolingdesk izrecno potrjujeta in se strinjata, da je programska oprema po svoji naravi lahko predmet napak, pomanjkljivosti, nepravilnega delovanja, prekinitev in nepričakovanih rezultatov ter da nobene programske opreme ni mogoče razumno zagotavljati kot popolnoma brezhibne v vseh okoliščinah.'
          : 'By registering, accepting these Terms or using Toolingdesk, the Customer and each User expressly acknowledge and agree that software by its nature may contain defects, deficiencies, malfunctions, interruptions and unexpected results and that no software can reasonably be guaranteed to be completely error-free in all circumstances.'}</P>
        <P>{sl
          ? 'V največjem obsegu, ki ga dovoljuje veljavno pravo, je Storitev, zlasti brezplačna in beta različica, zagotovljena »kot je« in »kot je na voljo«, brez jamstva, da bo vedno brez napak, neprekinjena, popolnoma varna, združljiva z vsemi sistemi ali primerna za vsak posamezen namen Naročnika.'
          : 'To the fullest extent permitted by applicable law, the Service, particularly the free and beta version, is provided "as is" and "as available", without any warranty that it will always be error-free, uninterrupted, completely secure, compatible with every system or suitable for every particular Customer purpose.'}</P>
        <P>{sl
          ? 'Naročnik izrecno sprejema, da lahko Toolingdesk vsebuje programske napake, napake v formulah ali logiki, nepravilne izračune, nepravilne prikaze, napake pri uvozu ali izvozu, nepravilno obdelavo podatkov, težave pri generiranju dokumentov ali druge pomanjkljivosti. Ponudnik ne daje jamstva, da so Predkalkulacije, Ponudbe, izračuni ali drugi Rezultati popolni, točni ali primerni za neposredno uporabo brez preverjanja. Naročnik se strinja, da obstoj napake v Storitev sam po sebi ne pomeni, da Ponudnik prevzema odgovornost za poslovne posledice uporabe napačnega Rezultata.'
          : 'The Customer expressly accepts that Toolingdesk may contain software bugs, errors in formulas or logic, incorrect calculations, incorrect displays, import or export errors, incorrect data processing, document-generation errors or other defects. The Provider does not warrant that Cost Estimates, Quotes, calculations or other Outputs are complete, accurate or suitable for direct use without verification. The Customer agrees that the existence of an error in the Service does not by itself mean that the Provider assumes liability for business consequences resulting from use of an incorrect Output.'}</P>

        <H3>{sl ? '27. člen – Obvezno preverjanje predkalkulacij in ponudb' : 'Article 27 – Mandatory verification of estimates and quotes'}</H3>
        <P>{sl
          ? 'Naročnik in Uporabnik sta izključno odgovorna za preverjanje pravilnosti vseh podatkov, izračunov in Rezultatov pred njihovo poslovno uporabo ali posredovanjem tretji osebi. Preverjanje vključuje zlasti količine, dimenzije, materiale, nabavne in prodajne cene, cene materiala, strojne urne postavke, strojne in proizvodne čase, pripravljalne čase, stroške dela, stroške orodij, zunanje storitve, režijske stroške, pribitke, popuste, marže, davke, valute, roke, komercialne pogoje in končno ponudbeno ceno.'
          : 'The Customer and User are solely responsible for verifying the correctness of all data, calculations and Outputs before business use or disclosure to a third party. Such verification includes quantities, dimensions, materials, purchase and sales prices, material prices, machine rates, machine and production times, setup times, labour costs, tooling costs, outsourced services, overhead, markups, discounts, margins, taxes, currencies, deadlines, commercial terms and final quoted price.'}</P>
        <P>{sl
          ? 'Naročnik se izrecno strinja, da Toolingdesk predstavlja pripomoček za predkalkulacijo in pripravo ponudb ter ne nadomešča strokovne kontrole tehnologa, kalkulanta, komercialista, proizvodnega strokovnjaka ali druge ustrezno usposobljene osebe. Preden Naročnik oziroma Uporabnik Ponudbo pošlje kupcu, sprejme naročilo, naroči material, začne proizvodnjo ali na podlagi Rezultata sprejme drugo poslovno odločitev, mora sam preveriti pravilnost relevantnih podatkov in Rezultata. Naročnik je odgovoren za končno vsebino, ceno in pogoje vsake Ponudbe, ki jo posreduje tretji osebi, ne glede na to, ali je bila Ponudba v celoti ali delno ustvarjena s Toolingdesk.'
          : 'The Customer expressly acknowledges and agrees that Toolingdesk is an assistance tool for cost estimation and quote preparation and does not replace professional review by a technologist, estimator, sales professional, manufacturing professional or other appropriately qualified person. Before sending a Quote to a customer, accepting an order, purchasing material, starting production or making another business decision based on an Output, the Customer or User must independently verify the relevant data and Output. The Customer is responsible for the final content, price and terms of every Quote delivered to a third party, whether generated wholly or partly using Toolingdesk.'}</P>

        <H3>{sl ? '28. člen – Napake uporabnika in napake programske opreme' : 'Article 28 – User errors and software errors'}</H3>
        <P>{sl ? 'V največjem obsegu, ki ga dovoljuje veljavno pravo, Ponudnik ne odgovarja za škodo, izgubo, stroške ali druge posledice, ki nastanejo zaradi:' : 'To the fullest extent permitted by applicable law, the Provider is not liable for loss, damage, costs or other consequences arising from:'}</P>
        <Ul items={sl ? [
        ...
        <P>{sl
          ? 'Naročnik izrecno potrjuje in se strinja, da je sam odgovoren za končno strokovno in komercialno kontrolo Rezultatov, tudi kadar je napaka v Rezultatu posledica programske napake Toolingdesk. Če Naročnik zazna očitno ali domnevno napako, Rezultata ne sme uporabiti za zavezujočo poslovno odločitev, dokler ga ne preveri oziroma napake ne odpravi ali ustrezno upošteva. Ta člen ne izključuje odgovornosti v obsegu, v katerem je po prisilnem pravu ni dovoljeno izključiti.'
          : 'The Customer expressly acknowledges and agrees that it remains responsible for final professional and commercial review of Outputs even where an incorrect Output results from a Toolingdesk software error. If the Customer identifies an apparent or suspected error, the Output must not be used for a binding business decision until independently verified and the error has been corrected or appropriately accounted for. This Article does not exclude liability to the extent exclusion is prohibited by mandatory law.'}</P>

        <H3>{sl ? '29. člen – Izključene vrste škode' : 'Article 29 – Excluded categories of loss'}</H3>
        <P>{sl
          ? 'V največjem obsegu, ki ga dovoljuje veljavno pravo, Ponudnik v nobenem primeru ne odgovarja za posredno, posledično, posebno, naključno ali kaznovalno škodo oziroma za izgubljeni dobiček, izgubljeni prihodek, izgubljeno maržo, izgubljeno poslovno priložnost, izgubljeno naročilo, izgubo kupca, izgubo ugleda, prekinitev poslovanja, stroške nadomestne proizvodnje, stroške ponovne izdelave, pogodbene kazni Naročnika do njegovih strank ali druge primerljive poslovne posledice. Izključitev velja ne glede na pravno podlago zahtevka in tudi, če je bil Ponudnik opozorjen na možnost nastanka take škode, kolikor je takšna izključitev po veljavnem pravu dopustna.'
          : 'To the fullest extent permitted by applicable law, the Provider shall in no event be liable for indirect, consequential, special, incidental or punitive damages, or for lost profits, lost revenue, lost margin, lost business opportunities, lost orders, loss of customers, reputational loss, business interruption, replacement-production costs, rework costs, contractual penalties owed by the Customer to its own customers or comparable business losses. This exclusion applies regardless of the legal theory of the claim and even where the Provider has been advised of the possibility of such loss, to the extent permitted by applicable law.'}</P>

        <H3>{sl ? '30. člen – Najvišja skupna odgovornost Ponudnika' : 'Article 30 – Maximum aggregate liability'}</H3>
        <P>{sl
          ? 'Naročnik izrecno potrjuje in se strinja, da v največjem obsegu, ki ga dovoljuje veljavno pravo, skupna kumulativna odgovornost Ponudnika iz ali v zvezi s Storitvijo v nobenem primeru ne presega skupnega zneska, ki ga je konkretni Naročnik dejansko plačal Ponudniku za uporabo Toolingdesk v dvanajstih (12) mesecih neposredno pred dogodkom, iz katerega izvira zahtevek. Omejitev velja za vse zahtevke skupaj, ne za vsak zahtevek posebej, in ne glede na to, ali zahtevek temelji na pogodbi, odškodninski odgovornosti, kršitvi zakonske obveznosti ali drugi pravni podlagi, kolikor je takšna omejitev dovoljena.'
          : 'The Customer expressly acknowledges and agrees that, to the fullest extent permitted by applicable law, the Provider\'s total cumulative liability arising out of or relating to the Service shall in no event exceed the total amount actually paid by that Customer to the Provider for use of Toolingdesk during the twelve (12) months immediately preceding the event giving rise to the claim. The cap applies to all claims in the aggregate, not separately to each claim, and applies regardless of whether a claim is based in contract, tort, breach of statutory duty or another legal theory, to the extent such limitation is permitted.'}</P>
        <P>{sl
          ? 'Če Naročnik za uporabo Storitev v relevantnem dvanajstmesečnem obdobju ni plačal nobenega zneska, je pogodbeno dogovorjena zgornja meja odgovornosti Ponudnika 0 EUR, vendar samo v obsegu, v katerem je takšna omejitev po veljavnem prisilnem pravu dopustna. Naročnik potrjuje, da je omejitev odgovornosti bistven element dogovora, zlasti glede na brezplačno oziroma cenovno omejeno naravo Storitev, in da Ponudnik brez teh omejitev Storitev pod enakimi pogoji ne bi zagotavljal.'
          : 'Where the Customer paid no amount for the Service during the relevant twelve-month period, the contractually agreed maximum liability of the Provider is EUR 0, but only to the extent such limitation is permitted under applicable mandatory law. The Customer acknowledges that these limitations are an essential basis of the agreement, particularly given the free or limited-price nature of the Service, and that the Provider would not provide the Service on the same terms without them.'}</P>
        <P>{sl
          ? 'Ne glede na besedilo »v nobenem primeru« nobena določba teh Pogojev ne izključuje ali omejuje odgovornosti, ki je po veljavnem prisilnem pravu ni mogoče vnaprej izključiti ali omejiti. To zlasti vključuje odgovornost za škodo, povzročeno namenoma ali iz hude malomarnosti, kadar tako določa veljavno pravo.'
          : 'Notwithstanding the words "in no event", nothing in these Terms excludes or limits liability that cannot lawfully be excluded or limited in advance under applicable mandatory law. This includes, in particular, liability for intentional misconduct or gross negligence where applicable law so provides.'}</P>

        <H3>{sl ? '31. člen – Višja sila' : 'Article 31 – Force majeure'}</H3>
        <P>{sl
          ? 'Ponudnik ne odgovarja za neizpolnitev, zamudo, izgubo razpoložljivosti ali motnjo, ki je posledica dogodkov zunaj njegovega razumnega nadzora, vključno z naravnimi nesrečami, vojno, terorizmom, stavkami, izpadi elektrike ali interneta, kibernetskimi napadi, epidemijami, ravnanjem državnih organov ter večjimi izpadi Supabase, Vercel ali drugih infrastrukturnih ponudnikov. Če okoliščina višje sile traja dalj časa, lahko Ponudnik prizadeto funkcionalnost začasno omeji ali Storitev prekine brez odgovornosti za posledice, ki jih po prisilnem pravu ni dolžan nositi.'
          : 'The Provider is not liable for failure, delay, loss of availability or disruption caused by events beyond its reasonable control, including natural disasters, war, terrorism, strikes, electricity or internet outages, cyberattacks, epidemics, government action and material outages of Supabase, Vercel or other infrastructure providers. If a force-majeure event continues for an extended period, the Provider may restrict affected functionality or discontinue the Service without liability for consequences it is not required to bear under mandatory law.'}</P>
        ...
        <H3>{sl ? 'Kategorije posameznikov in podatkov (36. člen)' : 'Data subjects and categories of data (Article 36)'}</H3>
        ...
        <H3>{sl ? 'Podobdelovalci (40. člen)' : 'Subprocessors (Article 40)'}</H3>
